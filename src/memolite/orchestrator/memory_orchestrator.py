...
from memolite.storage.session_store import SessionRecord, SqliteSessionStore
...
class MemoryOrchestrator:
    ...
    async def search_memories(
        ...
        *,
        ...
        resolved_session_id, resolved_semantic_set_id = await self._resolve_search_scope(
            session_key=session_key,
            session_id=session_id,
    ...
    async def _resolve_search_scope(
        ...
        *,
        ...
        resolved_semantic_set_id = semantic_set_id

        if session_key is None:
            return resolved_session_id, resolved_semantic_set_id
        ...
        if resolved_session_id is None:
            if session is not None:
                ...
                resolved_session_id = session_key
        if resolved_semantic_set_id is None:
            if session is not None:
                resolved_semantic_set_id = self._set_id_resolver(session)
            else:
                resolved_semantic_set_id = session_key
        return resolved_session_id, resolved_semantic_set_id

...
def test_memory_search_respects_runtime_memory_config(tmp_path: Path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        client.patch("/memory-config/episodic", json={"top_k": 1, "context_window": 0})
        client.patch("/memory-config/long-term", json={"semantic_enabled": False})
        ...
        assert len(payload["episodic_matches"]) == 1
...
def test_api_response_schema_for_memory_search_is_stable(tmp_path: Path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        assert set(payload["episodic_matches"][0].keys()) == {
            "episode",
            "derivative_uid",
            "score",
        }
        assert set(payload["episodic_matches"][0]["episode"].keys()) == {
            "uid",
            "session_key",
            "session_id",
            "producer_id",
        ...
        assert set(payload["combined"][0].keys()) == {
            "source",
            "content",
            "identifier",
            "score",
